 ...
 * @Last Modified time: 2021-09-07 13:17:22
 * width { Number } 基础宽度
 * src { String } 视频地址
 */
...
let vlcCanGetTimer = null;
let vlcTotal = 0;
let vlcTimer = null; //获取时间定时器
let vlc_t = 0;

let full_vlcInit_t = 0;
let vlcObResizeTimer = null;
let canChangeVlcSize = false;
...
const VlcObject = (props) => {
  ...
  const [crrTime, setCrrTime] = useState(0);
  //屏幕显示
  useEffect(() => {
    // 状态重置
    canChangeVlcSize = false;
    full_vlcInit_t = 0;
    ...
    setTimeout(() => {
      canChangeVlcSize = true;
    }, 10000);
    return () => {
      clearInterval(vlcInitialTimer);
      clearInterval(vlcTimer);
      clearInterval(vlcCanGetTimer);
      clearTimeout(vlcObResizeTimer);
    };
  }, []);
  //初始化海康设备执行函数
  function initialHK() {
    ...
    //刷时间
    let vlcCanGetTimer = setInterval(() => {
  ...
  function getVlcTime() {
    ...
    if (vlc) {
      ...
      if (time) {
        ...
        full_vlcInit_t = parseInt(_time);
        setCrrTime(parseInt(_time));
  ...
  function getVlcDHTime() {
    ...
    setCrrTime(vlc_t + vlcInitailTime);
    full_vlcInit_t = vlc_t + vlcInitailTime;
    pro = pro > 100 ? 100 : pro;
    setProgress(pro);
  }
  useEffect(() => {
    if (vlcObResizeTimer) clearTimeout(vlcObResizeTimer);
    vlcObResizeTimer = setTimeout(() => {
      if (!canChangeVlcSize) return;
      let url = getVlcUrl(full_vlcInit_t, src);
      setUrl(url);

      // 状态重置
      vlcInitailTime = full_vlcInit_t;
      setReload(true);
      setTimeout(() => {
        setReload(false);
        setTimeout(() => {
  ...
  const hendlePlayPause = useCallback(() => {
    ...
    if (vlc) {
      if (play) {
        ...
        setPlay(true);
        // vlc.playlist.play();
        console.log("full_vlcInit_t", full_vlcInit_t);
        let url = getVlcUrl(full_vlcInit_t, src);
        setUrl(url);
        // 状态重置
        vlcInitailTime = full_vlcInit_t;
  ...
  return (
    <div
      className="mzc-vlc"
      style={{
        width: full ? document.body.clientWidth : width,
        height: full ? document.body.clientHeight : height,
    ...
    >
      <div
        style={{
          ...
          height: full ? document.body.clientHeight - 50 : height - 50,
      ...
      >
        {reload ? null : err ? (
          ...
          <object
            ...
            hspace="500px"
            width={full ? document.body.clientWidth - 1 : width}
      ...
      <div className="mzc-vlcControl" style={{ width: full ? "100%" : width }}>
        ...
        <div className="mzc-vlcControlPlayback">
          ...
          <div className="mzc-vlcControlPlayback-operation">
            ...
            <div className="mzc-vlcControlPlayback-operationLeft">
              ...
              <span>
                {crrTime ? formatVlcTime(crrTime) : "00:00:00"} /{" "}
                {vlcTotal ? formatVlcTime(vlcTotal) : "00:00:00"}
              </span>
            </div>
            <div className="mzc-vlcControlPlayback-operationRight">
              ...
              <div className="mzc-vlcControlPlayback-operationVolume">
                <img
                  style={{ width: muted ? 20 : 17, height: muted ? 20 : 17 }}
