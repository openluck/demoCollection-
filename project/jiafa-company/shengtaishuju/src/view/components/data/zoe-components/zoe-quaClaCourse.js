 ...
 * @Last Modified by: tj
 * @Last Modified time: 2021-01-20 17:06:09
 * 教学秩序-课程统计-学院下课程统计页面
 */
...
class ZoeQuaClaCourse extends Component {
  constructor(props) {
    super(props);
    this.state = {
      collegeId: '', //开课单位id
  ...
  render() {
    ...
    const { collegeId, courseId, pinkInfo, pageNum, pageSize, QuaClaCourSta, curSign, isLoading } = this.props
    const orderTh = [
      {
        title: "开课单位",
    ...
    return (
      <div style={{ height: "calc(100% - 45px)" }}>
        <ZoeQuaStatis subPageType={1} curSign={2} />
        ...
        >
          <Table
            ...
            columns={orderTh}
            dataSource={QuaClaCourSta.list}
            pagination={false}
            rowKey={record => record.courseId}
          />

          <Fy
            total={QuaClaCourSta.total}
            pageIndex={pageNum}
            pageSize={pageSize}
...
export default ZoeQuaClaCourse;