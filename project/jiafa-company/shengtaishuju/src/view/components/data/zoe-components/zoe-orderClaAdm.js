 ...
 * @Last Modified by: tj
 * @Last Modified time: 2021-01-20 16:16:34
 * 教学秩序-课程统计-教学班统计页面
 */
...
class ZoeOrderClaAdm extends Component {
  constructor(props) {
    super(props);
    this.state = {
      collegeId: "", //开课单位id
  ...
  render() {
    const { ClaAdmSta, sortArr, pageNum, pageSize, curSign, isLoading } =
      this.props;
    const { roleType } = G.ISCED_curRoleInfo;
    console.log(ClaAdmSta);
    let curSignArray = G.ISCED_tabArray || [];
    const orderTh = [
      {
        title: "开课单位",
          ...
          },
      {
        ...
        className:
          G.ISCED_setInfo.ifClassroomDiscipline === "1" ? "" : "zoe-none",
        filterDropdown: true,
        render: (text) => {
          return text ? text + "%" : "0%";
        },
    ...
    return (
      <div style={{ height: "calc(100% - 45px)" }}>
        <ZoeOrderStatis subPageType={1} curSign={3} />
        ...
        >
          <Table
            ...
            className={
              "zoe-order-table zoe-order-cla-table1 zoe-order-table-click"
            }
            ...
            columns={orderTh}
            dataSource={ClaAdmSta.list}
            pagination={false}
            rowKey={(record) => record.teaClaId}
          />
          {(roleType === "1" || roleType === "2") && curSign !== 1 ? (
            <Fy
              total={ClaAdmSta.total}
              pageIndex={pageNum}
              pageSize={pageSize}
