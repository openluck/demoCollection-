...
export default class OneScreen extends Component {
    constructor(props) {
        ...
        let url = props.url || 'rtmp://10.4.3.47:1936/live|rtspstd_hik/10.4.2.141/554/1/main/admin/jfkj12345';
        this.state = {
            isLive: true,
            url,
            volume: 60,
    ...
    componentDidMount() {
        let { url } = this.state;
        {
            window.masterSendNetStreamHandle = () => {
                if (window.CKobject.getObjectById('rdd_1')) {
                    CKobject.getObjectById('rdd_1').videoPlay();
                }
            };
            window.loadedMaster = function (index) {
                window.CKobject.getObjectById('rdd_1').addListener(
                    "sendNetStream",
        ...
        document.querySelector(`.v1`) ? document.querySelector(`.v1`).className = `lean-video-mount v1` : null;
