...
export default function $axios(options) {
  return new Promise((resolve, reject) => {
    ...
    instance.interceptors.response.use(
      response => {
        ...
        if (response.headers && (response.headers['content-type'] === 'application/x-msdownload' || response.headers['content-type'] === 'application/vnd.ms-excel')) {
          ...
          // return {blob: response.data,url:response.request.responseURL}
          return response
        }
        // 根据返回的code值来做不同的处理
        switch (data.rc) {
    ...
    instance({ ...options, data: { ...config.data, data: { ...options.data, examId: sessionStorage.getItem('examId') } } }).then(res => {
      resolve(res)
      return false
    }).catch(error => {
      reject(error)
