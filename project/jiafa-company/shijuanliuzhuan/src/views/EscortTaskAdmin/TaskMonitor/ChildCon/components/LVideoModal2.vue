<template>
  <div class="video_modal">
    <a-modal
      v-model="appVideoModalVisible2"
      footer
      @cancel="appVideoModalVisible2 = false"
      wrapClassName="my-modal"
    >
      <!-- <div id="largeVideo2" v-if="isGetedVideo" ref="myref" style="width:100%; height:100%;">
...
export default {
  ...
  data() {
    return {
      ...
      videoObject: {
        ...
        container: "#largeVideo2", //“#”代表容器的ID，“.”或“”代表容器的class
  ...
  methods: {
    initFlvM2(url) {
      let that = this;
      let flvPlayerRef2 = document.getElementById("largeVideo2");
      if (flvjs.isSupported()) {
        this.flvPlayerObjM2 = flvjs.createPlayer(
          {
            type: "flv",
            isLive: true,
            url: url,
        ...
        this.flvPlayerObjM2.attachMediaElement(flvPlayerRef2);
        this.flvPlayerObjM2.load(); //加载
        this.flvPlayerObjM2.play();
        that.flvPlayerObjM2.on(flvjs.Events.STATISTICS_INFO, function (res) {
          ...
          if (this.lastDecodedFrame != res.decodedFrames) {
            ...
            if (that.flvPlayerObjM2) {
              that.flvPlayerObjM2.pause();
              that.flvPlayerObjM2.unload();
              that.flvPlayerObjM2.detachMediaElement();
              that.flvPlayerObjM2.destroy();
              that.flvPlayerObjM2 = null;
              that.initFlvM1(url);
            }
          }
        });
        // 断流重连
        that.flvPlayerObjM2.on(
          flvjs.Events.ERROR,
    ...
    playVideo(url) {
      if (this.flvPlayerObjM2 !== null) {
        this.flvPlayerObjM2.pause();
        this.flvPlayerObjM2.unload();
        this.flvPlayerObjM2.detachMediaElement();
        this.flvPlayerObjM2.destroy();
        this.flvPlayerObjM2 = null;
      }
      this.initFlvM2(url);
    },
    closeModal() {
      this.appVideoModalVisible2 = false;
    },
    getDefaultDevice() {
      if (this.aioDevice) {
        return this.aioDevice.deviceId;
