<template>
  <div class="header">
    <div class="heaader-content">
      <span @click="goInstructions"
        ><span style="margin-right: 5px">说明文档</span
        ><svg-icon iconClass="28" color="#409fff"
      /></span>
      <!-- <router-link target="_blank" to="/Instructions"
        ><span style="margin-right: 5px">说明文档</span
        ><svg-icon iconClass="28" color="#409fff"
      /></router-link> -->
      <span>
...
<script>
// import InstructionsUrl from "../public/config.js";
export default {
  data() {
    return {
      userName: "",
  ...
  mounted() {
    ...
    // }, 500);
    this.InstructionsUrl = window.G.InstructionsUrl;
  },
  methods: {
    logout() {
      window.close();
    ...
    goInstructions() {
      ...
      window.open(this.InstructionsUrl, "_blank"); // 在新窗口打开外链接
      // window.location.href = this.InstructionsUrl; //在本页面打开外部链接
    },
  },
};
</script>
