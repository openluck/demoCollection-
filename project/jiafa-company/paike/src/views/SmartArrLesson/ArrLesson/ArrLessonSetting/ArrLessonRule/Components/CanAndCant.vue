  ...
  <div class="can-and-cant">
    <div class="selection">
      ...
      <template v-if="baseData.rules == 8 || baseData.rules == 9">
        ...
        <div
          class="select-item"
          v-if="baseData.rules == 8 || baseData.rules == 9"
        >
          <label for=""
            >{{
    ...
    <div class="operation">
      ...
      >
        <a-button
          type="primary"
          :disabled="hasMaskingRuleId"
          @click="handleSetHTTime"
        >
          确认任意时间{{
            baseData.rules == 10
              ? "合班"
              : requestParamsHT.ruleType == 8
              ? "互斥"
              : "同步"
          }}
        </a-button>
...
export default {
  ...
  watch: {
    ...
    "requestParamsHT.setMutuxTime"(setMutuxTime) {
      ...
      if (
        setMutuxTime == false &&
        ((usePerson1 && usePerson2) || (crrCourse1 && crrCourse2))
      ) {
        this.GetTeacherMutexSync();
      }
    },
