...
<template>
  <div class="student-timetable">
    ...
    <div class="right">
      <div class="select">
        筛选：<a-checkbox-group
          :disabled="!selectedKeys.length || tableLoading"
          :value="checkedList"
            ...
            <div :key="index">
              ...
              >
                ...
                <div class="little-unit">
                  <!-- <span v-for="(o, q) in item.lesTeacher" :key="q">{{
                  o.name
                }}</span> -->
                  <span
                ...
                <div class="little-unit" v-if="filtrateStatus.class">
                  {{ item.classes }}
                </div>
              </div>
            </div>
  ...
  methods: {
    ...
    onLoadData(treeNode) {
      return new Promise(async (resolve) => {
        ...
        const list = await this.getStudentsTree(
          treeNode.dataRef.orgId,
          treeNode.dataRef.orgType + 1
        );
    ...
    async getStudentsTree(placeId, orgType) {
      ...
      const data = await this.$api.Common.getStudentsTree(params);
    ...
    async getStudentsTimetable(stuId) {
      const arrLessonId = this.arrLessonId;
      this.tableLoading = true;
      const data = await this.$api.PreviewTimetable.getStudentsTimetable({
        stuId,
        arrLessonId,
      });
      if (data.code == 200) {
