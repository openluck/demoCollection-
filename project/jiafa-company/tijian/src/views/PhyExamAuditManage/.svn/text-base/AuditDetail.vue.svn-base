...
export default {
  ...
  async mounted() {
    ...
    this.bus.$emit(
      "changeSelectedKey",
      "/phyExamAuditManage/croAreaAudit"
    );
  },
  methods: {
    filterArray,
    goBack() {
      this.$router.push("/phyExamAuditManage/croAreaAudit");
    },
    // 获取表格高度
    getTableHeight() {
    ...
    async getList() {
      ...
      try {
        const res = await this.$api.auditDetail.getExamineeList({
          ...this.search,
        });
    ...
    async getExamMemLocationTree() {
      try {
        const res = await this.$api.auditDetail.examMemLocationTree({
        ...
        if (res.code === "200" || res.code === 200) {
          ...
          // console.log(res.data, this.parentCode);
          // console.log(this.examMemLocationTree);
        } else {
          this.$message.error(res.message);
        }
      } catch (error) {
    ...
    async getPhyExamLocationTree() {
      try {
        const res = await this.$api.auditDetail.phyExamLocationTree({
          applyId: this.search.applyId,
        });
        if (res.code === "200" || res.code === 200) {
          if (res.data.length) {
    ...
    async exportExcel() {
      ...
      try {
        const res = await this.$api.auditDetail.exportExaminee({
          ...this.search,
        });
        // console.log(res);
        downloadFile(res);
