...
<!-- 复检考生查询  -->
  <div id="recheckExamineeQuery">
    ...
    <div class="list">
      ...
      >
        <!-- 复核结果 -->
        <span slot="recheckResult" slot-scope="text">
          <a @click="handlePreview(text)">复查结果</a>
        </span>
        <!-- 复检说明 -->
        <span slot="recheckExplain" slot-scope="text">
        ...
        <!-- 考生状态 -->
        <span slot="examState" slot-scope="text" :class="text==='4'?'colGray':text==='5'?'colGreen':''">
          {{ text==='4'?'待复检':text==='5'?'复检完成':'-' }}
        </span>
        <!-- 详情 -->
    ...
    <a-modal
      v-model="visible" 
      title="考生复检详情" 
      @ok="recheckDetailsModalHandleOk"
      width="660px"
      :maskClosable="false"
      wrapClassName="recheckDetailsModal"
    >
    <template slot="footer">
      <a-button type="primary" @click="recheckDetailsModalHandleOk" style="margin-right:10px;">
        <svg-icon icon-class="queren" :scale="0.8" style="margin-right:5px;" />
        确认
      </a-button>
      <a-button @click="recheckDetailsModalHandleCancel">
      ...
      <span class="title">复检详情</span>
      <div class="recheckDetails">
        <div class="line">
          <div class="left">复检科室：</div>
          <div class="right">{{examDetails.recheckDesk}}</div>
        </div>
        <div class="line">
          <div class="left">复检说明：</div>
          <div class="right">{{examDetails.recheckExplain}}</div>
        </div>
        <div class="line">
          <div class="left">复检结果：</div>
          <div class="right">
            <img :src="examDetails.recheckResult" alt="复检结果" style="width:150px; cursor: pointer;" @click="showImg(examDetails.recheckResult)">
          </div>
        </div>
      </div>
    </a-modal>
...
const columns = [
  ...
  {
    title: "复检说明",
    dataIndex: "recheckExplain",
    width: 250,
    key: "recheckExplain",
    scopedSlots: { customRender: "recheckExplain" },
    ellipsis: true
  },
  {
    title: "复检结果",
...
export default {
  ...
  methods: {
    ...
    //详情modal框确定
    recheckDetailsModalHandleOk(e) {
      console.log(e);
      this.visible = false;
    },
    //详情modal框取消
    recheckDetailsModalHandleCancel(e) {
      console.log(e);
      this.visible = false;
    },
    ...
    async getList() {
      ...
      try {
        const res = await this.$api.PhyExamQueryStat.recheckExamineeList({
          ...this.search
        });
        if (res.code === "200" || res.code === 200) {
          console.log(res);
