<template>
  <div id="ConfigManage">
    <div v-if="String(orgType) === '1'" class="province">
      ...
      <div class="list">
        ...
        >
          ...
          <template slot="editItem" slot-scope="text, record">
            <span
              v-for="(item, index) in record.editItem"
              :key="index"
              class="item"
            >
          ...
          <template slot="checkItem" slot-scope="text, record">
            <span
              v-for="(item, index) in record.checkItem"
              :key="index"
              class="item"
            >
        ...
        >
          <template slot="item" slot-scope="text, record">
            <a-checkbox
              ...
              @change="
                (checkedValues) => changeCheckbox(checkedValues, item, record)
              "
            >
              {{ item.itemName }}
            </a-checkbox>
    ...
    <div v-else class="county">
      <!-- <div class="switch" style="margin-bottom:15px">
        <span>是否开启体检日程分组：</span>

        <a-popconfirm
          @confirm="setSwitch"
        >
          <a-switch
          :checked="switchState==='1'?true:false"
          @change="changeSwitch" 
        />
          <template slot="title">
            <span>确认{{switchState==='1'?'关闭':'开启'}}日程分组功能？</span>
          </template>
        </a-popconfirm>
      </div> -->

      <div class="inform" style="margin-bottom: 15px">
        <div class="text" style="margin-bottom: 15px">
          <span>体检考生通知设置：</span>
        </div>
        <div class="textarea">
          <a-textarea
            placeholder="请输入相关通知内容"
            v-model="inform"
            style="width: 800px; minheight: 300px; maxheight: 300px"
          />
        </div>
      </div>

      <div class="bottom">
        <a-button type="primary" @click="informSave">
          <svg-icon
            icon-class="baocun"
            :scale="0.8"
            style="margin-right: 5px"
        ...
        <!-- <a-button type="primary" @click="informSend">
          <svg-icon icon-class="fasong" :scale="0.8" style="margin-right:5px;" />
          发送
        </a-button> -->
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      // switchState: "", // 开关状态
      inform: "", //通知内容
      // switchChecked: undefined // 开关改变后的状态
    };
  },
  mounted() {
  ...
  methods: {
    ...
    async getEditList() {
      try {
        const res = await this.$api.configurationManage.getEditList({
          orgCode: this.orgCode,
        });
        if (res.code === "200" || res.code === 200) {
          console.log("可编辑项列表", res.data.list);
    ...
    async getDeskList() {
      ...
      try {
        const res = await this.$api.configurationManage.getDeskList({
          orgCode: this.orgCode,
        });
        if (res.code === "200" || res.code === 200) {
          if (res.data.list) {
            console.log("科室列表", res.data.list);
            var dataList = res.data.list;
    ...
    addEditable(record) {
      ...
      console.log(record.editItem, record.checkItem, record.deskName);
      this.visible = true;
      this.addType = "1";
      ...
      console.log("所有可编辑项", this.allEdit);
      ...
      console.log("已选择编辑项", this.selectedEdit);
      //临时存储胸透肝功（加禁用）
      this.editItemIdTemp = "";
    ...
    addCheckItem(record) {
      ...
      console.log(record, record.deskName);
      this.visible = true;
      this.addType = "2";
      this.deskId = record.deskId;
    ...
    handleOk() {
      this.visible = false;
      console.log(this.addType, this.deskId, this.addEdit);
      if (this.addType === "1") {
        console.log(this.dataSource);
    ...
    changeCheckbox(checkedValues, item, record) {
      console.log(checkedValues.target.checked, item, record);
      if (this.addType === "1") {
        var itemObj = {};
        ...
        if (checkedValues.target.checked) {
          ...
          this.addEdit = this.addEdit.filter(
            (i) => i.editItemId !== item.itemId
          );
        }
        console.log("新增的项", this.addEdit);
      } else if (this.addType === "2") {
        var itemObj1 = {};
        itemObj1.checkItemId = item.itemId;
        itemObj1.checkItemName = item.itemName;

        if (checkedValues.target.checked) {
          this.addCheck.push(itemObj1);
        } else {
          this.addCheck = this.addCheck.filter(
            (i) => i.checkItemId !== item.itemId
          );
        }
    ...
    async saveConfig() {
      try {
        for (let i = 0; i < this.dataSource.length; i++) {
          ...
          let arrTemp = this.dataSource[i].checkItem.concat(
            this.dataSource[i].editItem
          );
          for (let j = 0; j < this.dataSource[i].editItem.length; j++) {
            ...
            if (element.editItemId === "jzsl") {
              ...
              let resultT = arrTemp.find((item) => {
                return (
                  item.checkItemId === "lysl" || item.editItemId === "lysl"
                );
              });
              if (!resultT) {
                this.$message.error(
        ...
        for (let index = 0; index < dataList.length; index++) {
          ...
          for (let j = 0; j < this.dataSourceTemp.length; j++) {
            ...
            if (dataList[index].deskId === element.deskId) {
              console.log(dataList[index].deskName, element.deskName);
        ...
        const res = await this.$api.configurationManage.saveConfig({
          orgCode: this.orgCode,
          deskList: dataList,
        });
        if (res.code === "200" || res.code === 200) {
    ...
    // 获取通知内容
    async getStateAddInform() {
      try {
        const res = await this.$api.configurationManage.getStateAddInform({
          orgCode: this.orgCode,
        });
        if (res.code === "200" || res.code === 200) {
          console.log(res.data);
    ...
    /* async setSwitch(){
      try {
        const res = await this.$api.configurationManage.switchSet({
          orgCode: this.orgCode,
          switchState: `${this.switchChecked * 1}`
        });
        if (res.code === "200" || res.code === 200) {
    ...
    async informSave() {
      try {
        const res = await this.$api.configurationManage.informSave({
          orgCode: this.orgCode,
          inform: this.inform,
        });
        if (res.code === "200" || res.code === 200) {
