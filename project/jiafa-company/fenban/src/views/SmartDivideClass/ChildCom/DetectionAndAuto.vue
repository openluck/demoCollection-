  ...
  <div class="detection-and-auto">
    ...
    >
      <div class="container" style="margin-top:10px">
        ...
        <div class="right">
          <div v-if="count !== 0">
            检测到您还有<span class="num">{{ count }}</span
            >人还未进行分班，是否继续分班？
          </div>
          <div v-else >
            检测到您已完成<span style="color: #1ba4b3">{{
              divideclassType === "1"
                ? "选考"
                : divideclassType === "2"
                ? "学考"
            ...
            >分班，<span v-if="divideclassType === '1'">可进行<span style="color: #1ba4b3">学考分班</span>，也</span
            >可直接发布该方案！
          </div>
          <div class="tips" v-if="divideclassType === '1' && count === 0">
            <div class="tips-con">
              <a-icon
                type="info-circle"
                style="font-size: 22px; color: #ffaa00"
              />
              <div style="margin-left:10px">可选择不进行学考分班直接发布，则排课时无学考班级</div>
            </div>
          </div>
        </div>
      </div>
      <div v-if="count !== 0" slot="footer">
        <a-button :disabled="btnsDisabled" @click="cancel">取 消</a-button>
        <a-button
          class="yellowBtn"
          @click="handleAutoDivideClass"
          type="primary"
          >自动分班
        </a-button>
        <a-button
