 ...
 * @Last Modified by: tj
 * @Last Modified time: 2021-01-20 17:12:09
 * 教学质量-教师统计-教学班下所有课堂数据
 */
...
class ZoeQuaMasRoom extends Component {
  constructor(props) {
    super(props);
    this.state = {};
  }
  ...
  clickRow = (record) => {
    ...
    this.props.history.push(`/home/data/quatea/${record.claRoomId}`)
  ...
  render() {
    const { pageNum, pageSize, QuaMasRoomSta } = this.props;
    const orderTh = [
      {
        title: "开课单位",
    ...
    return (
      <div style={{ height: "calc(100% - 45px)" }}>
        <ZoeQuaStatis subPageType={2} curSign={4} />
        ...
        >
          <Table
            ...
            // loading={isLoading}
            className={"zoe-qua-tea-table zoe-order-table-noclick"}
            columns={orderTh}
            dataSource={QuaMasRoomSta.list || []}
            pagination={false}
            rowKey={record => record.claRoomId}
          />
          <Fy
            total={QuaMasRoomSta.total || 0}
            pageIndex={pageNum}
            pageSize={pageSize}
...
export default  withRouter(ZoeQuaMasRoom);
