 ...
 * @Last Modified by: tj
 * @Last Modified time: 2021-03-22 13:47:31
 */
...
/************************************ 目录 ************************************/
/**
 * setSS ----------- 在sessionStorage中存一个加密过后的值
 * getSS ----------- 从sessionStorage中取一个值，并且解密
...
/**
 * 
 * @param {String} selTime 月份时间 2021-03
 * @description 获取开始时间和结束时间
 */
const getMonthDate = (selTime) => {
  let timeArray=[]
  if (selTime) {
    let time=selTime.split('-')
    let tempT=time.map(item=> {return parseInt(item)})
    let lastD=new Date(tempT[0], tempT[1], 0).getDate()
    timeArray[0]=selTime+'-01'
    timeArray[1]=selTime+'-'+lastD;
  }
  return timeArray
}
