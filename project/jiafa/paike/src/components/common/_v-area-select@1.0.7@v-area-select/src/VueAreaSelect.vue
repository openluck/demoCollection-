  ...
  methods: {
    ...
    handleMouseDown(e) {
      ...
      const pageX =
        clientX +
        scrollLeft +
        (!this.rootElType ? document.documentElement.scrollLeft : 0);
      const pageY =
        clientY +
        scrollTop +
        (!this.rootElType ? document.documentElement.scrollTop : 0);
      ...
      if (this.chooseDirectly) {
        this.$nextTick(() => {
          ...
          let selectedItem = e.target.getAttribute("data-selected-index");
          this.$emit("handleClick", selectedItem);
        });
      }
    },
    handleMouseMove(e) {
    ...
    handleAreaSelect() {
      ...
      for (let i = 0; i < this.selectDoms.length; i++) {
        ...
        if (!flag && !this.preventParent && selectedIndex > -1)
          selectingList.splice(index, 1);
      }
    ...
    collide(rect, rectSelect) {
      const maxX = Math.max(
        rect.x + rect.width,
        rectSelect.x + rectSelect.width
      );
      const maxY = Math.max(
        rect.y + rect.height,
        rectSelect.y + rectSelect.height
      );
      const minX = Math.min(rect.x, rectSelect.x);
      const minY = Math.min(rect.y, rectSelect.y);
      return Boolean(
        maxX - minX <= rect.width + rectSelect.width &&
          maxY - minY <= rect.height + rectSelect.height
      );
    },
    ...
    autoScroll(pageX, pageY, clientX, clientY, interval) {
      ...
      this.scrollTimer = setInterval(() => {
        ...
        if (directX === "left") {
          ...
          this.rootEl.scrollLeft =
            scrollLeft <= 0 ? 0 : this.rootEl.scrollLeft - 50;
        }
        if (directX === "right") {
          pageX = scrollWidth;
          ...
          this.rootEl.scrollLeft = flag
            ? scrollWidth - clientWidth
            : this.rootEl.scrollLeft + 50;
        }
        if (directY === "up") {
          pageY = 0;
          ...
          this.rootEl.scrollTop =
            scrollTop <= 0 ? 0 : this.rootEl.scrollTop - 50;
        }
        if (directY === "down") {
          pageY = scrollHeight;
          ...
          if (flag) clearInterval(this.scrollTimer);
          this.rootEl.scrollTop = flag
            ? scrollHeight - clientHeight
            : this.rootEl.scrollTop + 50;
        }
        this.checkParent(pageX, pageY);
      }, interval);
    },
    getClassNameList() {
      let list =
        typeof this.selectItemClassName === "string"
          ? this.selectItemClassName.split(",")
    ...
    getAncestorDom() {
      ...
      const flag =
        acestorDom === document.documentElement || acestorDom === document;
      const type = flag ? 1 : 0;
    ...
    getAreaClassName() {
      this.selectAreaDom =
        this.selectAreaClassName === ""
          ? this.$el.parentNode
          : document.querySelector(`.${this.selectAreaClassName}`);
    },
  },
  watch: {
    selectedList: {
