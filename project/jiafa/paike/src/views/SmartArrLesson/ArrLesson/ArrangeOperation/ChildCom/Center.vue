 ...
 * @LastEditTime: 2021-10-14 17:05:39
-->
  ...
  methods: {
    ...
    async autoSchedul() {
      ...
      if (res.code === "200") {
        ...
        let btnList = [];
        let errors = [];
        //1验证完成无冲突(无按钮), 2课程未完成-新增(重新排课按钮) 3教师未完成(手动/自动)-新增(重新排课按钮) 4教师冲突-新增(有按钮) 5规则冲突-新增(有按钮) 6场所冲突(无按钮)
        switch (type) {
          case 1:
            if (this.isAutoResetPkBtn) {
              txt =
                "该课表已校验完成，无任何人员或场所的时间冲突，可直接发布该课表；可进行课表预览或重新进行排课（重新排课只清空掉自动排课结果）";
              btnList = [
          ...
          case 2:
            if (this.isAutoResetPkBtn) {
              txt =
                "您的排课未完成，请对剩余课程继续排课；可进行重新进行排课（重新排课只清空掉自动排课结果）";
              btnList = [
                { btnName: "继续手动排课", btnClick: "", btnId: "41" },
                { btnName: "重新排课（清空）", btnClick: "", btnId: "42" },
              ];
            ...
            break;
          case 3:
            // txt = "您的排课未完成，请对剩余教师继续排课；可进行重新进行排课（重新排课只清空掉自动排课结果）";
            // btnList = [
            //   { btnName: "继续手动排课", btnClick: "", btnId: "41" },
            //   { btnName: "自动排课", btnClick: "", btnId: "42" },
            // ];
            if (this.isAutoResetPkBtn) {
              txt =
                "您的排课未完成，请对剩余教师继续排课；可进行重新进行排课（重新排课只清空掉自动排课结果）";
              btnList = [
                { btnName: "继续手动排课", btnClick: "", btnId: "41" },
                { btnName: "重新排课（清空）", btnClick: "", btnId: "42" },
              ];
          ...
          case 4:
            ...
            if (this.isAutoResetPkBtn) {
              txt =
                "您的排课存在教师冲突，可进行重新进行排课（重新排课只清空掉自动排课结果）";
              btnList = [
                { btnName: "继续手动排课", btnClick: "", btnId: "41" },
                { btnName: "重新排课（清空）", btnClick: "", btnId: "42" },
              ];
          ...
          case 5:
            ...
            if (this.isAutoResetPkBtn) {
              txt =
                "您的排课存在规则冲突，可进行重新进行排课（重新排课只清空掉自动排课结果）";
              btnList = [
                { btnName: "继续手动排课", btnClick: "", btnId: "41" },
                { btnName: "重新排课（清空）", btnClick: "", btnId: "42" },
              ];
