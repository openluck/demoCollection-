 ...
 * @LastEditors: cb
 * @LastEditTime: 2021-10-11 17:00:29
-->
<template>
  <div class="cb-container">
     <div class="common-title">学生换课</div>
     <a-skeleton :loading="skeletonLoading">
     </a-skeleton>
     <div v-show="!skeletonLoading" class="cb-p-filter">
       <div>
          <FilterStu ref="FilterStu"  @filterTimetable='filterTimetable' />
       </div>
       <div class="cb-p-btn">
        <a-button v-if="isHave" @click="openRecord">
          <svg-icon icon-class="com_adjust_record" :scale="0.8" style="margin-right:8px"></svg-icon>
          调整记录
        </a-button>
        <a-button v-if="isHave" @click="saveTimeTable" type="primary">
     ...
     <div v-show="!skeletonLoading">
        ...
        >  
          <template #suffixIcon>
            <svg-icon icon-class="com_calendar" style="margin-top:-8px"></svg-icon>          
          </template>
        ...
        <!-- 条件过滤 -->
        <a-checkbox-group :options="options" style="margin-left: 49px;" :value="filterValue" @change="onChange" />
     </div>
     </div>
     <!-- 课表 -->
     <div v-show="!skeletonLoading">
       <div v-if="isHave"  class="cb-p-list">
       <div @click="getWeek('1')" class="cb-p-left" >
         <svg-icon class="gSvg" icon-class="com_cg_week" :scale="0.8"></svg-icon>
       </div>
        <div @click="getWeek('2')" class="cb-p-right">
           <svg-icon class="gSvg" icon-class="com_cg_week"  :scale="0.8"></svg-icon>
        </div>     
     <!-- 课表 -->
      <a-table
      ...
      >
        <!--标题名字  -->
        <span v-for="(item,index) in slotList" :key='index' :slot="item">
           {{filertWeek(item)}} 
           <span v-if="slotDay[index]" style="margin-left:11px;color: #797c7f;">( {{slotDay[index]}} )</span>
        </span>
        <!-- 课表内容 -->
      </a-table>
     <TableCell v-if="false"/>
     </div>
     <div v-else class="cb-p-empty">
        <a-empty
        ...
        >
          <span slot="description" style="color:#616366;">暂无任何数据，请创建学生课表</span>
          <a-button type="primary" @click="addTimeTable">
            <svg-icon icon-class="com_add"  style="margin-right:8px"></svg-icon>
            创建学生课表
          </a-button>
        </a-empty>
     </div>
     ...
     <StuModal @partObj='partObj' ref="StuModal"/>
     <RecordModal ref="RecordModal" />
     <SelectCourseModal @addCommit='addCommit' ref="SelectCourseModal"/>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async getCgableCourseList(type) {
      const res = await this.$api.studentAdjustment.getCgableCourseList({ 'classId': this.searchData.classId });
      if (res.code === '200' && res.result) {
        if (res.data.length > 0) {
