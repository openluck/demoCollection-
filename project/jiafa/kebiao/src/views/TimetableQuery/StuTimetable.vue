 ...
 * @LastEditTime: 2021-10-20 09:32:32
 * @LastEditors: YanQY
 * @Description: 学生课表
 * @FilePath: \Web\src\views\TimetableQuery\StuTimetable.vue
-->
<template>
  <div class="ylc-class-content">
    ...
    <section>
      <div class="head">
        <div class="select">
          <div class="ylc-select-title">学生课表</div>
          <!-- 学年 -->
          ...
          <!-- 班级列表选择 -->
          <a-select v-model="fetchData.classId"
            style="width: 130px; margin-left: 16px"
            @change="classChange">
            <a-select-option v-for="item of classList"
              :value="item.classId"
              :key="item.classId">{{ item.className }}</a-select-option>
          </a-select>
      ...
      <div style="display: flex">
        <div class="ylc-class-list">
          <div class="ylc-search-div">
            <svg-icon icon-class="com_search"
              class="ylc-search-icon" />
            <a-input placeholder="学生姓名"
              class="ylc-search-inp"
              v-model="searchStr" />
          </div>
          <div v-for="item in stuShowList"
            v-show="item.stuId !== ''"
            :key="item.stuId">
            <div :class="
              item.stuId === fetchData.personId
              ? 'ylc-class-item ylc-checked'
              : 'ylc-class-item'
              "
              @click="chooseStu(item.stuId)">
              {{ item.stuName }}
            </div>
          </div>
        </div>
...
export default {
    ...
    searchStr(val) {
      if (val) {
        ...
        this.stuList.map((item) => {
          if (item.stuName.indexOf(val) !== -1) {
            this.stuShowList.push(item);
          }
        });
      } else {
        this.stuShowList = this.stuList;
      }
    }
  },
  computed: {
    ...mapState("timetableQuery", ["stuTimeTable"]),
    columns() {
      let arr = [
        {
          title: "时段",
  ...
  methods: {
    ...mapMutations("timetableQuery", ["setStuTimeTable"]),
    moment,
    // 初始化数据
    async init() {
      this.schoolYearList = JSON.parse(sessionStorage.getItem("schoolYearList"));
      this.height = (window.innerHeight - 210) + "px"
      if (this.stuTimeTable.secId) {
        this.fetchData.schoolYearId = this.stuTimeTable.schoolYearId
        this.fetchData.semesterId = this.stuTimeTable.semesterId
        this.fetchData.secId = this.stuTimeTable.secId
        this.fetchData.gradeId = this.stuTimeTable.gradeId
        this.fetchData.classId = this.stuTimeTable.classId
        this.semesterList = this.stuTimeTable.semesterList
        this.defaultWeek = this.stuTimeTable.defaultWeek
        this.secList = this.stuTimeTable.secList
        this.gradeList = this.stuTimeTable.gradeList
        this.classList = this.stuTimeTable.classList
      } else {
      ...
      this.secList.map((item) => {
        if (e === item.secId) {
          ...
          if (this.classList.length > 0) {
            ...
            this.classList = []
            return this.$message.warn("暂无班级信息")
          }
        }
      });
      this.setData()
    ...
    async getTeachWeek(teachWeek) {
      ...
      await this.getStuByClass()
      this.setData()
    },
    // 查询
    search() {
      if (this.fetchStatu) {
        return this.getStuByClass();
      }
      this.semesterList.map((item) => {
    ...
    cancel() {
      this.stuShowList = this.stuList;
      if (this.fetchData.stuId === "") {
        this.fetchData.stuId = this.stuList[0].stuId;
      }
      this.visible = false;
    },
    // 确定导入
    ...
    async exportTimeTable() {
      const res = await this.$api.StuTimetable.exportStudentTimetable({
        ...this.fetchData,
        studentIdList: this.fetchData.personId === "" ? [] : [this.fetchData.personId],
    ...
    async getStuByClass() {
      ...
      const res = await this.$api.common.getStuByClass({
        ...this.fetchData
      });
      if (res.code === "200" || res.code === 200) {
        this.stuList = res.data;
        this.stuShowList = res.data;
        if (this.stuList.length > 0) {
          this.fetchData.personId = this.stuList[0].stuId;
          this.getStuTable()
        } else {
          this.stuList = []
          this.stuShowList = []
          return this.$message.warn("暂无学生信息");
        }
      } else {
        this.$message.warn(res.message);
      }
    ...
    // 获取学生课表
    async getStuTable() {
      if (!this.fetchData.personId || !this.fetchData.classId || !this.fetchData.teachWeekId) {
        return
      }
