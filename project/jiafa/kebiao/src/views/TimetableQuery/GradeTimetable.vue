...
<<<<<<< .mine
 * @LastEditTime: 2021-10-12 17:19:37
 * @LastEditors: OpenLcuk
||||||| .r50390
 * @LastEditTime: 2021-10-12 09:44:44
 * @LastEditors: ylc
=======
 * @LastEditTime: 2021-10-12 17:15:02
 * @LastEditors: ylc
>>>>>>> .r50393
 * @Description: 年级课表
...
export default {
  ...
  methods: {
    ...
    dataLength() {
      ...
      let SunArr = [];
      this.data.map((item) => {
        MonArr.push(item.lesMon[0] ? item.lesMon[0].lesList.length : 0);
        TueArr.push(item.lesTue[0] ? item.lesTue[0].lesList.length : 0);
        WedArr.push(item.lesWed[0] ? item.lesWed[0].lesList.length : 0);
        ThuArr.push(item.lesThu[0] ? item.lesThu[0].lesList.length : 0);
        FriArr.push(item.lesFri[0] ? item.lesFri[0].lesList.length : 0);
        SatArr.push(item.lesSat[0] ? item.lesSat[0].lesList.length : 0);
        SunArr.push(item.lesSun[0] ? item.lesSun[0].lesList.length : 0);
      });
    ...
    // 获取年级课表
    async getGradeTable() {
      if (!this.fetchData.gradeId) return
