 ...
 * @LastEditTime: 2021-10-20 09:35:18
 * @LastEditors: ylc
 * @Description: 教师课表
 * @FilePath: \Web\src\views\TimetableQuery\TechTimetable.vue
-->
<template>
  <div class="ylc-class-content">
    ...
    <section>
      <div class="head">
        <div class="select">
          <div class="ylc-select-title">教师课表</div>
          <!-- 学年 -->
          ...
          <!-- 科目列表选择 -->
          <a-select v-model="fetchData.subjectId"
            @change="setSubject"
            style="width: 120px; margin-left: 16px">
            <a-select-option v-for="item of subjectList"
      ...
      <div style="display: flex">
        <div class="ylc-class-list">
          <div class="ylc-search-div">
            <svg-icon icon-class="com_search"
              class="ylc-search-icon" />
            <a-input placeholder="教师姓名"
              class="ylc-search-inp"
              v-model="searchStr" />
          </div>

          <div v-for="item in teacherShowList"
            v-show="item.teacherId !== ''"
            :key="item.teacherId"
            :class="
                  item.teacherId === fetchData.personId
                    ? 'ylc-class-item  ylc-checked'
                    : 'ylc-class-item'
                "
            @click="chooseTeacher(item.teacherId)">
            {{ item.teacherName }}
          </div>
        </div>
    ...
    <GlobalModal :visible="visible"
      ...
      <div style="padding: 56px 64px">
        <div style="display: flex">
          <div style="height: 32px; line-height: 32px">教师范围：</div>
          <div>
            <a-select style="width: 240px"
              v-model="fetchData.teacherId">
              <a-select-option v-for="item in teacherShowList"
...
export default {
  ...
  data() {
    return {
      ...
      subjectList: [], // 科目
      teacherList: [], // 教师列表
      teacherShowList: [], // 教师展示
      tableLoading: false, // 表格加载
      checkList: ["科目", "场所", "班级"], // 显示项列表
      lesWeek: [
        "lesMon",
        "lesTue",
        "lesWed",
  ...
  watch: {
    searchStr(val, oldVal) {
      if (val) {
        this.teacherShowList = [];
        this.teacherList.map((item) => {
          if (item.teacherName.indexOf(val) !== -1) {
            this.teacherShowList.push(item);
          }
        });
      } else {
        this.teacherShowList = this.teacherList;
      }
    }
  },
  computed: {
    ...mapState("timetableQuery", ["teacherTimeTable"]),
    columns() {
      let arr = [
        {
          title: "时段",
  ...
  methods: {
    ...mapMutations("timetableQuery", ["setTeacherTimeTable"]),
    moment,
    // 初始化数据
    async init() {
      this.schoolYearList = JSON.parse(sessionStorage.getItem("schoolYearList"));
      this.height = (window.innerHeight - 210) + "px"
      if (this.teacherTimeTable.secId) {
        this.fetchData.schoolYearId = this.teacherTimeTable.schoolYearId
        this.fetchData.semesterId = this.teacherTimeTable.semesterId
        this.fetchData.secId = this.teacherTimeTable.secId
        this.fetchData.subjectId = this.teacherTimeTable.subjectId
        this.semesterList = this.teacherTimeTable.semesterList
        this.subjectList = this.teacherTimeTable.subjectList
        this.defaultWeek = this.teacherTimeTable.defaultWeek
        this.secList = this.teacherTimeTable.secList
      } else {
    ...
    setSubject(e) {
      this.fetchStatu = false
      this.fetchData.subjectId = e;
      this.setData()
    },
    // 获取教学周
    async getTeachWeek(teachWeek) {
    ...
    // 点击导出
    handleExport() {
      this.fetchData.teacherId = this.fetchData.personId
      ...
      if (this.teacherList.length > 0) {
        this.visible = true;
        this.teacherShowList.push({
          teacherId: "",
          teacherName: "全部"
        });
      } else {
        this.$message.warn("暂无教师信息");
      }
    },
    // 导出取消
    cancel() {
      this.teacherShowList = this.teacherList;
      if (this.fetchData.teacherId === "") {
        this.fetchData.teacherId = this.teacherList[0].teacherId;
      }
      this.visible = false;
    },
    // 确定导出
    ...
    async exportTimeTable() {
      const res = await this.$api.TechTimetable.exportTeacherTimetable({
        ...this.fetchData,
        teacherIdList:
          this.fetchData.teacherId === "" ? [] : [this.fetchData.teacherId],
    ...
    async getSubjectList() {
      if (!this.fetchData.secId) {
        return
      }
      const res = await this.$api.common.getSubjecByStudySec({
        ...this.fetchData
      });
      if (res.code === "200" || res.code === 200) {
        this.subjectList = res.data;
        this.fetchData.subjectId = this.subjectList[0].subjectId;
      } else {
        this.$message.warn(res.message);
      }
    },
    ...
    async getTeacherList() {
      ...
      const res = await this.$api.common.getTeacherBySub({
        ...this.fetchData
      });
      if (res.code === "200" || res.code === 200) {
        this.teacherList = res.data;
        this.teacherShowList = res.data;
        if (this.teacherList.length > 0) {
          this.fetchData.personId = this.teacherList[0].teacherId;
          this.getTeacherTable()
        } else {
          this.$message.warn("暂无教师信息")
        }
    ...
    async getTeacherTable() {
      if (!this.fetchData.secId || !this.fetchData.subjectId || !this.fetchData.personId || !this.fetchData.teachWeekId) {
        return
      }
