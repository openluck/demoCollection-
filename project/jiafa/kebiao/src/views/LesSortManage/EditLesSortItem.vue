 ...
 * @LastEditors: YanQY
 * @LastEditTime: 2021-09-22 17:21:11
-->
<template>
  <div
    class="yqy-edit-lesSort-item"
    ref="yqy-edit-lesSort-item"
  >
    <header class="back">
      <div class="back-left">
      ...
      <div class="back-right">
        <a-button @click="toPage('/LesSortManage/lesSortSetting')">取消</a-button>
        <a-button
          type="primary"
          @click="save()"
        >下一步</a-button>
    ...
    >
      ...
      <div class="list">
        <div
          class="list-item"
          :class="moveIndex[0]===index?'moving-down':moveIndex[1]===index?'moving-up':''"
        ...
        >
          ...
          <div class="type-box box">
            ...
            <span v-if="item.lesSortType==='1'" class="box-text">授课 ({{item.lesType==="0"?"非自习":"自习"}})</span>
            <span v-else-if="item.lesSortType==='0'" class="box-text">非授课</span>
          </div>
          <div class="show-box box">
            <span class="box-title">展示形式</span>
            <span class="box-text">{{item.showType==="1"?"通栏":"七列"}}</span>
          </div>
          ...
          <div class="move-box">
            <svg-icon
              ...
              @click.native="index===0?()=>{}:swapArray(lesSortList,index,index-1)"
            >
            </svg-icon>
            <svg-icon
              ...
              @click.native="index===lesSortList.length-1?()=>{}:swapArray(lesSortList,index,index+1)"
            >
            </svg-icon>
          </div>
        </div>
...
export default {
  ...
  async created() {
    ...
    if (isBack === 't') {
      ...
      if (this.lesSortPlanId) {
        await this.getLesSortPlanInfo({ secId: this.curSec.secId, lesSortPlanId: this.lesSortPlanId })
      }
    }
  },
  mounted() {
  ...
  methods: {
    ...
    addItem() {
      this.itemData = {
        // lesSortId: "",
        lesSortIndex: this.lesSortList.length ? this.lesSortList[this.lesSortList.length - 1].lesSortIndex + 1 : 1,
        lesSortName: "",
        lesSortType: "1",
        lesType: "0",
        showType: "2",
    ...
    delItem(data) {
      ...
      this.lesSortList = this.lesSortList.filter(i => i.lesSortIndex !== data.lesSortIndex)
        .map(i => {
    ...
    async getLesSortPlanInfo(data) {
      ...
      try {
        ...
        if (res.code === '200' && res.result) {
          ...
          this.$store.commit('lesSort/changeOriginalLesSortList', JSON.parse(JSON.stringify(this.lesSortList)))
