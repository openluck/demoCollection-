 ...
 * @LastEditors: YanQY
 * @LastEditTime: 2021-11-10 13:26:01
-->
<template>
  <div
      ...
      <div class="content-left">
        ...
        >
          ...
          >
            ...
            <a-date-picker
              v-model="form.dateList[0]"
              format="MM-DD"
              style="width:150px;z-index:9"
              :disabled="onlyShow"
              valueFormat="MM-DD"
              :getCalendarContainer="(v) => v.parentNode"
              :showToday="false"
            ...
            >
              <template v-slot:suffixIcon>
                <svg-icon
                  icon-class="menu_2"
                  style="margin-top: -7px;width: 14px;"
                />
              </template>
            </a-date-picker>{{" ~"}}
            <a-date-picker
              v-model="form.dateList[1]"
              format="MM-DD"
              style="width:150px;z-index:9"
              :disabled="onlyShow"
              valueFormat="MM-DD"
          ...
          >
            ...
            >
              ...
              <div class="box-text">
                ...
                <span v-else>{{formatTime(item.beginTime)}} ~
            ...
            <div class="spacing-box box">
              ...
              <span class="box-text">
                {{computedHoursMins(item.beginTime,item.endTime).h+"时"+computedHoursMins(item.beginTime,item.endTime).m+"分"}}
                <!-- {{computedHoursMins(item.beginTime,item.endTime).m}} 分钟 -->
              </span>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    async onLoadData({ dataRef }) {
      ...
      if (gradeId) {
        //如果是年级
        const children = await this.getClassByGrade({ gradeId, secId, appId });
    ...
    judgeTimeChange(orgList, newList) {
      for (let i = 0, j = orgList.length; i < j; i++) {
        if (
          orgList[i].beginTime !== newList[i].beginTime ||
    ...
    async save() {
      ...
      if (!this.lesSortList.every((i) => i.beginTime && i.endTime)) {
        timeResult = false;
      }
      ...
      if (timeResult && nameResult && dataResult && applyReslut && !repeatName) {
        ...
        const arr = lesSortList
          .map((i) => ({
            ...
            beginTime: i.beginTime.slice(0, 5),
            endTime: i.endTime.slice(0, 5)
          }))
          .filter((i) => i.beginTime || i.endTime);
        const stanArr = arr.filter((i) => i.beginTime || i.endTime);
    ...
    inRange(item, targetTime) {
      ...
      let beginTime = item.beginTime.slice(0, 5); //.slice(0, 5) 去掉秒钟数
      let endTime = item.endTime.slice(0, 5);
      let tarStart = targetTime.beginTime.slice(0, 5);
      let tarEnd = targetTime.endTime.slice(0, 5);
      if (beginTime === tarStart || endTime === tarEnd) {
        return false
      }
      if (beginTime < tarStart && tarStart < endTime) {
        return false;
      } else if (beginTime < tarEnd && tarEnd < endTime) {
        return false;
      } else if (tarStart < beginTime && beginTime < tarEnd) {
        return false;
