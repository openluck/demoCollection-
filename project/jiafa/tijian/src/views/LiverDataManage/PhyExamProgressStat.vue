<template>
  <div class="PhyExamProgressStatXRay">
    <div class="PhyExamProgressStatXRayT">
      ...
      <div class="searchBtn">
        <a-button type="primary" @click="getList()"> 查 询 </a-button>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      try {
        const res = await this.$api.PhyExamProgressStatL.getList({
          ...this.search,
        });
        if (res.code === 200) {
    ...
    async getExport() {
      ...
      try {
        const res = await this.$api.PhyExamProgressStatL.getExport(data);
        if (res) {
          downloadFile(res);
        } else {
        }
