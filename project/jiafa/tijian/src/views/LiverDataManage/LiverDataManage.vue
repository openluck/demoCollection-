    ...
    <div class="list">
      ...
      <div slot="transaminaseResult"  slot-scope="text">
        ...
        <div slot="action"  slot-scope="text, record">
          <a-button :record="record" @click="vision(record)" >{{ record.isResult === '已下结论' ? "详情" : "编辑" }}</a-button>
    ...
    >
      ...
      <div class="XRayForm">
        ...
        <div class="XRayFormItem">
          ...
          <a-input-number style="margin-right: 10px" :disabled="disabled"  v-model="studentItem.transaminaseNum" :min="80" :max="9999"  />U/L
        </div>
        <div class="XRayFormItem">
      ...
      <div class="XRayForm">
          <!-- <a-radio-group  :disabled="disabled" v-model="studentItem.doctorOptionId">
      ...
      <div>
        <div class="modalDoctor">
          <div style="margin-right:50px">
            ...
            <div v-if="studentItem.doctorName">总检医生 : {{studentItem.doctorName}}</div>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      userCode: '',
      doctorOptionIds: [],
      commitTitle: '请确认是否要对当前体检医院范围内全部[转氨酶]未体检的考生进行批量提交操作？',
      orgTitle: '',
    };
  },
  created() {
  ...
  methods: {
    ...
    filterDisabled(item) {
      if (item.deskSta === '已体检' && item.isResult === "未下结论") {
        return false
      } else {
        return true
      }
    ...
    orgChange(value, e) {
      // console.log(e.componentOptions.propsData.title);
    ...
    changePopTitle() {
      if (!this.orgTitle && !this.search.date) {
        this.commitTitle = '请确认是否要对当前体检医院范围内全部[转氨酶]未体检的考生进行批量提交操作？'
      } else if (this.orgTitle && this.search.date) {
        this.commitTitle = '请确认是否要对当前体检医院范围内' + this.orgTitle + '下' + this.search.date + '日[转氨酶]未体检的考生进行批量提交操作？'
      } else if (!this.orgTitle && this.search.date) {
        this.commitTitle = '请确认是否要对当前体检医院范围内' + this.search.date + '日[转氨酶]未体检的考生进行批量提交操作？'
      } else if (this.orgTitle && !this.search.date) {
        this.commitTitle = '请确认是否要对当前体检医院范围内' + this.orgTitle + '下[转氨酶]未体检的考生进行批量提交操作？'
      }
    },
    async getList() {
      ...
      try {
        ...
        // }
        const res = await this.$api.LiverDataManage.getList(this.search);
        if (res.code === 200) {
