 ...
 * @Last Modified time: 2020-11-17 16:25:41
 * 数据服务请求方法
 */
...
// 用于听评课的请求方法
const requestForListen = function (method, params, success, fail) {
  let token = sessionStorage.getItem("token");
  let orgcode = sessionStorage.getItem("orgCode");
  let roleInfo = JSON.parse(sessionStorage.getItem("roleInfo"));
  let roleLevel = Number(roleInfo.roleLevel);
  ...
  if (method) {
    if (opts.testmode) {
      ...
      params.orgId = baseinfo.collegeId || ''
      var postData = {
        version: opts.version,
        data: params || {},
        alias: opts.alias,
 ...
 */
const setConfig_2 = function (dataService, token, orgcode) {
