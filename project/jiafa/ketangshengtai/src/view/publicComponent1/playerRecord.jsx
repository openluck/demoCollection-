 ...
 * @Last Modified time: 2020-11-19 13:12:30
 * 录播组件
 */
...
export default class PlayerRecord extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      visible: false,
      shouldComeNum: 0,
      statTime: moment(new Date()).format('HH:MM:SS'), // 统计时间 
    };
  }
          ...
          this.setState(
            {
              ...
              statTime: moment(res.data.statisticsTime || new Date()).format('HH:MM:SS')
              // statTime: format(new Date(res.data.statisticsTime || new Date()), "HH:mm:ss")
            });
        } else {
          this.setState({
            teaStatus: 0,
            ...
            showRobootInfo: true,
            shouldComeNum: 0,
            statTime: moment(new Date()).format('HH:MM:SS')
            // statTime: format(new Date(), "HH:mm:ss")
          });
        }
      }
    );
