 ...
 * @Last Modified by: yh
 * @Last Modified time: 2019-12-04 11:30:56
 */
...
export default class Multiscreen extends React.Component {
  constructor(props) {
    ...
    //循环多次  创建一个随机id
    Array.from({ length: playUrl.length }).map((index) => {
      // if(index === 0) {
      //   return;
      // }
  ...
  insPlayer(index = 0, url) {
    ...
    window.CKobject.embedSWF(
      ...
      this.state.playerWidth[index+1], //宽高
      this.state.playerHeight[index+1],
      flashvars,
      params
    );
  }
  ...
  windowResize = () => {
    ...
    this.setState({
      ...
      Childid && Childid.map((item, i) => {
        this.computeStyle(i+1, true)
        let video = CKobject.getObjectById(item); //判断ck播放器是否存在
        if (video) {
          CKobject.getObjectById(item).width = this.state.playerWidth[i+1];
          CKobject.getObjectById(item).height = this.state.playerHeight[i+1];
        }
      })
    })
  }
  ...
  componentDidMount() {
    ...
    if (this.state.getUrlError) {
      ...
      for (let i = 0; i < playUrl.length; i++) {
        if (!playUrl[i] || playUrl[i]._url_ == undefined) {
          // console.log("不渲染第", i, "个接口");
          continue;
        }
        document.querySelector(`.v${i + 2}`).className = `lean-video-mount v${i + 2}`;
    ...
    //ckplayer的全局事件注册，只做事件触发监听，不做任何逻辑处理，逻辑放在组件内的函数调用里
    {
      ...
      window.masterPausedHandle = (isPaused = false) => {
        ...
        Childid&&Childid.map((item,i)=>{
          let videoCon = window.CKobject.getObjectById
          if (videoCon) {
            if (isPaused) {
              console.log('1111');
      ...
      window.bufferHandle = function (e) {
        console.log('e', playing, e);
        // const { isLive, Childid } = _this.state;
        // if (!isLive) {
        //   if (!playing && e >= 100) {
        //     playing = true;
        //     console.log('e111', e, playing);
        //     Childid.map((item, index) => {
        //       if (window.CKobject.getObjectById(item)) {
        //         CKobject.getObjectById(item).videoPlay();
        //       }
        //     })
        //   } else if (playing && (typeof e === 'undefined' || e < 100)) {
        //     console.log('e2222', e);
        //     Childid.map((item, index) => {
        //       playing = false;
        //       if (window.CKobject.getObjectById(item)) {
        ...
        // } else {
        //   _num++;
        //   if (playing && (typeof e === 'undefined' || e < 100 || _num < 3)) {
        //     console.log('e2', e);
        //     Childid.map((item, index) => {
        //       playing = false;
        //       if (window.CKobject.getObjectById(item)) {
        ...
        //       }
        //     });
        //   } else if (!playing && e >= 100 && _num >= 3) {
        //     console.log('e1', e, playing);
        //     playing = true;
        //     Childid.map((item, index) => {
        //       if (window.CKobject.getObjectById(item)) {
        //         CKobject.getObjectById(item).videoPlay();
        ...
        {this.state.getUrlError ? (
          ...
          >
            ...
            <div id="video-content" style={this.computeStyle(0, isBGModal)}>
              ...
              />
              <div
                className="lean-video v3"
                id={DOMid[1]}
                style={this.computeStyle(2)}
              />
              <div
                className="lean-video v4"
                id={DOMid[2]}
                style={this.computeStyle(3)}
              />
              {/* <div className="lean-video vm" /> */}
              <ul>
                <li className="player-modal " style={this.computeStyle(10)} />
                <li className="player-modal " style={this.computeStyle(11)} />
                <li className="player-modal" style={this.computeStyle(12)} />
                <li className="player-modal " style={this.computeStyle(13)} />
                {/* <li className="modal modal-5" />
            <li className="modal modal-6" /> */}
              </ul>
            </div>
          )}
      </div>
