 ...
 * @param {String} key 要合并的列名
 *  @param {String} index 行序号0开始
 **/
let str = null;
const mergeCellKey = (text, data, key, index) => {
  ...
  if (index !== 0 && text.wkId === data[index - 1][key].wkId) {
    return 0;
  }
  if (text.wkId === str) {
    return 0;
 ...
 * @param {Array} data 表格数据
 * @param {String} key 要合并的列名
 * @param {String} keyStr 根据报名号、wkId.....进行合并
 *  @param {String} index 行序号0开始
 **/
let str2 = null;
const mergeCellKey2 = (text, data, key, keyStr, index) => {
  // debugger
  if (!text) {
    return 1;
  }
  ...
  if (index !== 0 && text === data[index - 1][key][keyStr]) {
    return 0;
  }
  if (text === str2) {
    return 0;
 ...
 **/
let str3 = null;
const mergeCellKey3 = (text, data, key, keyStr, index) => {
  // debugger
  if (!text) {
    return 1;
  }
  ...
  if (index !== 0 && text === data[index - 1][keyStr]) {
    return 0;
  }
  if (text === str3) {
    return 0;
...
function encrypt(word, keyStr) {
  keyStr = keyStr || "0765E81FBE62ACAE";
...
export {
  ...
  encrypt,
  decrypt,
  getQueryString, //获取地址栏中的查询参数
  getRouterString, //获取路由地址
