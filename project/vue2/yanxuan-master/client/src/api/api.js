...
class Https {
   ...
   *   size: 每页数据量
   * */
  getTopicData = (parmas) => HttpUtils.get("/topic/list1", parmas);
   ...
   *   id: 专题id
   * */
  getTopicDetailRelated = (parmas) => HttpUtils.get("/topic/related1", parmas);
   ...
   *   size
   * */
  getBrandList = (parmas) => HttpUtils.get("/brand/list", parmas);
  /*
   ...
   */
  getGoodsSku = (parmas = {}) => HttpUtils.get("/goods/sku", parmas);
  /*
   * 相关商品
   * /api/goods/related
   * parmas:
  ...
  //oderlist
  postOrderList = (params = {}) =>
    HttpUtils.post("/orderlist/getOrderList", params);
  //pushOrderList
  postPushOrder = (params = {}) =>
    HttpUtils.post("/orderlist/pushOrderList", params);
  postCreateUser = (params = {}) => HttpUtils.post("/auth/createuser", params);
  //getUserInfo
  getUserInfo = () => HttpUtils.get("/user/info");
  //setUserInfo
  postUserInfo = (params = {}) => HttpUtils.post("/user/setUserInfo", params);
  //收货
  postOrderSh = (params = {}) =>
    HttpUtils.post("/orderlist/postOrderSh", params);
  //pic
  postPic = (data = {}) => HttpUtils.uppic("/upload/uppic", data);

  getFoot = (params = {}) => HttpUtils.get("/footprint/list", params);
  /**
   *
   * @param params footprintId
   * @returns {Promise<unknown>}
   */
  postDelFoot = (params = {}) => HttpUtils.post("/footprint/delete", params);

  /**
   *
   * @param params {id,comment}
   * @returns {Promise | Promise<unknown>}
   */
  postComment = (params = {}) =>
    HttpUtils.post("/orderlist/postComment", params);

  /**
   *
   * @param parmas {keyword}
   * @returns {Promise | Promise<unknown>}
   */
  getGoodsSearch = (parmas = {}) =>
    HttpUtils.get("/search/searchGoods", parmas);
}
