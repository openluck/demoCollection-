<template>
  ...
  >
    ...
    <div class="scroll-list-wrap" v-if="isAddress !== 2">
      ...
      <div v-if="isAddress === 3" class="shoplist-title" ref="shoptitle">
        推荐商家
      </div>
      <DropdownMenu
        v-if="isAddress === 3"
    ...
    <div class="blank" v-if="isAddress === 2">
      ...
      <p>输入地址之后才能订餐哦</p>
      <van-button color="#ffd84d" type="primary" @click="gotoAddressDetail"
        >手动选择地址</van-button
      >
    </div>
  </div>
...
export default class Home extends Vue {
  ...
  async getShoppingList(cb?: any) {
    try {
      ...
      this.totalPage =
        this.total < this.limit ? 1 : Math.ceil(this.total / this.limit);
  ...
  async addressDetail(QMap, coords) {
    //获取地理位置
  ...
  scroll($event) {
    ...
    // @ts-ignore
    let menuTop =
      this.$refs.shoptitle.offsetTop + this.$refs.shoptitle.clientHeight - 55;
